...
以通話 UUID 維護程序內、具上限與逾時的最近四輪上下文，協助辨識局部追問與換句話
說；事實來源仍只有本輪命中的單筆已發布標準答案。通話中不可切換模式，掛電話、
明確結束語或靜默結束後清除上下文。自然生成失敗或輸出守門不通過時回退核准原文。

語音客服測試進入聆聽狀態後，若連續 8 秒沒有有效 ASR 輸入，瀏覽器會要求後端播放受控的 `check_in` 提示「還有什麼事可以協助您的嗎?」；再次等待 8 秒仍無有效輸入，則播放受控的 `farewell` 提示「很高興為你服務，歡迎您再度來電，再見。」並結束通話。端點只接受這兩個階段值，不接受瀏覽器提供任意朗讀文字。

## 資料與稽核

原始音訊只在瀏覽器與 MLX Audio 記憶體串流處理，orchestrator 不接收或保存音訊。ASR 逐字稿不寫入一般稽核 log。`voice_synthesis` 稽核事件只記 Turn ID、TTS 模型、句子／音訊片段數、首段與完整延遲及固定錯誤類型；`voice_playback` 另記插話模式、duck／confirm 延遲與誤觸次數。兩者都不記逐字稿、回答或音訊。僅在 development 且明確開啟診斷設定時，`/voice-test` 另以 session ID 記錄非敏感測試問答；個資輪次遮蔽內容，正式環境不生效。
